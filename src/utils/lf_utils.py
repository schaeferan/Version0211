...
class Plucker(LightField):
  """A class encapsulating the Plucker utilities."""
  ...
  def ray2lightfield(self, rays):
    ...
    lf_samples = jnp.concatenate([rays.directions, moments], axis=-1)
    non_intersect_mask = jnp.array([False] * lf_samples.shape[0])[:, None]
    return lf_samples, non_intersect_mask
...
def get_lightfield_obj(lf_config):
  ...
  elif lf_config.name == "plucker":
    lightfield_obj = Plucker(lf_config)
  else:
